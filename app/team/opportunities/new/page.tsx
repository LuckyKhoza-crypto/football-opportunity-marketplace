...
export default function NewOpportunityPage() {
  ...
  const addSecondaryPosition = (pos: string) => {
    if (!formData.secondary_positions.includes(pos)) {
      updateField("secondary_positions", [...formData.secondary_positions, pos]);
    }
  };
  ...
  const validate = (isPublishing: boolean): boolean => {
    ...
    if (formData.playing_level && !PLAYING_LEVELS.includes(formData.playing_level as typeof PLAYING_LEVELS[number])) {
    ...
    if (isPublishing) {
      if (!formData.title.trim()) {
        newErrors.title = "Title is required to publish";
      }
    }
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="mb-2 text-3xl font-bold">Post a Player Opportunity</h1>
          <p className="text-lg text-muted-foreground">
            Describe exactly who your team needs
          </p>
        </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    {POSITIONS.filter(
                      (p) => p !== formData.position && !formData.secondary_positions.includes(p),
                    ).map((pos) => (
                      <SelectItem key={pos} value={pos}>
                        {pos} - {POSITION_LABELS[pos]}
                      </SelectItem>
