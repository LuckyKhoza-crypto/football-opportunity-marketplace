...
export default async function PublicPlayerProfilePage({
  ...
  // Fetch the player profile (must be discoverable)
  ...
  // Fetch the user's full name and avatar
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="grid gap-6">
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {typedProfile.travel_radius && (
                  <div>
                    <p className="text-sm text-muted-foreground">Travel Radius</p>
                    <p className="font-medium">{typedProfile.travel_radius} miles</p>
                  </div>
                )}
              </div>

              {(typedProfile.willing_to_travel || typedProfile.willing_to_relocate) && (
          ...
          {/* Previous Clubs */}
          {typedProfile.previous_clubs && typedProfile.previous_clubs.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
              ...
              <CardContent>
                <div className="space-y-4">
                  {typedProfile.previous_clubs.map((club: PreviousClub, index: number) => (
                    <div key={index} className="border-b pb-4 last:border-b-0 last:pb-0">
                      ...
                      {club.achievements && (
                        <p className="mt-1 text-sm text-muted-foreground">{club.achievements}</p>
                      )}
                    </div>
                  ))}
                </div>
          ...
          {/* Stats */}
          {typedProfile.stats && Object.keys(typedProfile.stats).length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
                  {typedProfile.stats.appearances !== undefined && (
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary">{typedProfile.stats.appearances}</p>
                      <p className="text-xs text-muted-foreground">Appearances</p>
                    </div>
                  )}
                  {typedProfile.stats.goals !== undefined && (
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary">{typedProfile.stats.goals}</p>
                      <p className="text-xs text-muted-foreground">Goals</p>
                    </div>
                  )}
                  {typedProfile.stats.assists !== undefined && (
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary">{typedProfile.stats.assists}</p>
                      <p className="text-xs text-muted-foreground">Assists</p>
                    </div>
                  )}
                  {typedProfile.stats.cleanSheets !== undefined && (
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary">{typedProfile.stats.cleanSheets}</p>
                      <p className="text-xs text-muted-foreground">Clean Sheets</p>
                    </div>
                  )}
                  {typedProfile.stats.manOfTheMatch !== undefined && (
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary">{typedProfile.stats.manOfTheMatch}</p>
          ...
          {/* Achievements */}
          {typedProfile.achievements && typedProfile.achievements.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <Award className="h-5 w-5 text-primary" />
              ...
              <CardContent>
                <ul className="space-y-2">
                  {typedProfile.achievements.map((achievement: string, index: number) => (
          ...
          {/* Preferred Leagues */}
          {typedProfile.preferred_leagues && typedProfile.preferred_leagues.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {typedProfile.preferred_leagues.map((league: string, index: number) => (
                    <span key={index} className="inline-flex rounded-full bg-secondary px-3 py-1 text-sm">
                      {league}
                    </span>
                  ))}
                </div>
          ...
          {typedProfile.compensation_expectation && (
            <Card>
              ...
              <CardContent>
                <p className="text-muted-foreground">{typedProfile.compensation_expectation}</p>
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            <CardContent className="py-4">
              <p className="text-center text-xs text-muted-foreground">
                Player profile shown for recruitment purposes only.
              </p>
            </CardContent>
          </Card>
        </div>
